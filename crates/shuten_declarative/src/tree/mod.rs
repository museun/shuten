...
impl serde::Serialize for Tree {
        ...
        S: serde::Serializer,
    {
        ...
        use crate::external::serialize_arena::SerializeArena;
        let mut ser = serializer.serialize_struct("Tree", 4)?;
        ser.serialize_field("nodes", &SerializeArena(&*self.inner.nodes.borrow()))?;
...
impl std::fmt::Debug for Tree {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Tree")
            .field(
                "nodes",
                &crate::external::ArenaPrinter(&*self.inner.nodes.borrow()),
            )
...
impl Tree {
    ...
    pub fn get_current(&self) -> Ref<'_, Node> {
        ...
        Ref::map(nodes, |nodes| &nodes[current.0])
    }
...
impl Tree {
    pub(crate) fn start(&self) {
        ...
        let root = &mut nodes[self.inner.root.0];
        root.next = 0;
    }
    ...
    pub(crate) fn begin_widget<T: Widget>(&self, props: T::Props<'_>) -> Response<T::Response> {
        ...
        let (id, mut widget) = {
            ...
            if let Some(id) = Self::next_existing_widget(&mut nodes, parent) {
                let node = nodes.get_mut(id.get()).unwrap();
...
impl Tree {
    fn next_existing_widget(nodes: &mut Arena<Node>, pid: WidgetId) -> Option<WidgetId> {
        let parent = nodes.get_mut(pid.0).unwrap();
    ...
    fn new_widget<T: Widget>(
        ...
        let id = WidgetId(nodes.insert(Node {
    ...
    fn trim_children(nodes: &mut Arena<Node>, removed: &mut Vec<WidgetId>, id: WidgetId) {
        ...
        while let Some(cid) = queue.pop_front() {
            removed.push(cid);
            queue.extend(nodes.remove(cid.0).unwrap().children);
        }
    }

    fn remove(nodes: &mut Arena<Node>, removed: &mut Vec<WidgetId>, id: WidgetId) {
...
impl TreeInner {
    fn new() -> Self {
        ...
        let root = WidgetId(nodes.insert(Node {
